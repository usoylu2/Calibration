...
filter_aug_train = train_time_calibration(probability=0.5, Freq=Freq_mismatch, Focus=Focus_mismatch,
                                          Power=Power_mismatch, Noise=Noise_FLAG, filter_length=Filter_length,
                                          device="cuda:0")
test_images = [(750)*2]
us_images = [(1000)/4*5]
epochs = [20]
...
if __name__ == '__main__':
    ...
    for train_num, epoch, test_num, LR in zip(us_images, epochs, test_images, learning_rate):
        ...
        for i in range(repetition):
            ...
            temp, auc = test_function(x_test, y_test, depth_test, mean_data, std_data, PATH)
            print(temp, auc)
            c1.append(temp)
            c2.append(auc)
