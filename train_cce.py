...
from netmodel import MNet, MNetdense
...
filter_aug_test = test_time_calibration(Freq=Freq_mismatch, Focus=Focus_mismatch, Power=Power_mismatch,
                                        Noise=Noise_FLAG, device="cuda:0")
filter_aug_train = train_time_calibration(probability=1, Freq=Freq_mismatch, Focus=Focus_mismatch,
                                          Power=Power_mismatch, Noise=Noise_FLAG, device="cuda:0")
test_images = [(750)*2]
us_images = [(1000)/4*5]
epochs = [20]
repetition = 4
        ...
        for data in test_loader:
            ...
            for label, prediction, depth_index in zip(labels, predictions, depth):
                if label == prediction:
                    ...
                    depth_matrix[depth_index, 0] = depth_matrix[depth_index, 0] + 1
                else:
                    depth_matrix[depth_index, 1] = depth_matrix[depth_index, 1] + 1
...
def train_function(x_train, x_valid, y_train, y_valid, depth_train, depth_valid, PATH, epoch_num, LR):
    ...
    y_train_gpu = torch.from_numpy(y_train).long().to(device)
    ...
    y_valid_gpu = torch.from_numpy(y_valid).long().to(device)
    ...
    criterion = nn.CrossEntropyLoss(torch.tensor([1, 1]).float().to(device))
...
if __name__ == '__main__':
    ...
    for train_num, epoch, test_num, LR in zip(us_images, epochs, test_images, learning_rate):
        ...
        for i in range(repetition):
            ...
            mean_data, std_data, acc = train_function(x_train, x_valid, y_train, y_valid, depth_train, depth_valid,
                                                      PATH, epoch, LR)
            print("Testing:")
            print(x_test.shape, y_test.shape)
            temp = test_function(x_test, y_test, depth_test, mean_data, std_data, PATH)
            print(temp)
            c1.append(temp)
